...
void outfft (OutAction what, double data);
...
void usage (char *msg, char *arg, int longhelp)
{
    ...
    fprintf (SdifStdErr, "\n\n"
"Usage: sdifextract [options] [infile [outfile]] LISP VERSION\n"
"\n"  
...
outbpf (OutAction what, double data)
{
	...
	case BeginFrame:    time = data; fprintf (out, "(");			break;
			    
	case BeginMatrix:  fprintf (out, "("); break;
	case EndMatrix:    fprintf (out, ")"); break;
	case EndFrame:	   fprintf (out, ")"); break;
	default:	    /* do nothing */			break;
			    
	case BeginRow:	    fprintf (out, "(%f (", time);	break;
	case InRow:	        fprintf (out, "%10f ", data);	break;
	case EndRow:	    fprintf (out, "))");		break;
	case CloseFile:	    fprintf (out, ")"); fclose (out);			break;
...
outformat (OutAction what, double data)
{
	...
	case BeginFrame:   time = data;		fprintf (out, "(");			 break;
	case BeginMatrix:  fprintf (out, "(%d %f ", (int) data, time);	 break;
			   
	case BeginRow:	   rowcount = 0;	fprintf (out, "(");			 break;
	case InRow:	   rowcount++;	 fprintf(out, "%-12g ", data);	 break;
	case EndRow:	   if (rowcount) fprintf(out, ")");		 break;
...
/* PWGL */
void 
outfft (OutAction what, double data)
{
    static FILE	  *out = NULL;
    static double time = -1;
	...
	fprintf (out, "(:open-file \n");
	break;

	case BeginFrame:    time = data; fprintf (out, "(");break;
			    
	case BeginMatrix:  fprintf (out, "(:beg-matrix\n"); break;
	case EndMatrix:    fprintf (out, ":end-matrix)\n"); break;
	case EndFrame:	   fprintf (out, ":end-frame)\n"); break;
	default:	    /* do nothing */			break;
	...
	case EndRow:	    // fprintf (out, ":end-row))\n");		
						break;
	
	
	
	case CloseFile:	    fprintf (out, ":close-file)\n"); fclose (out);			break;
...
int main(int argc, char** argv)
{
	...
	if (argv [i][0] == '-')
	{
		...
		else if (SdifStrEq (argv [i], "-fft"))   output = outfft;
...
#		define boolopt  "hqvF"	/* flags with no arguments */
	...
	current_frame_time = -123456789.0;
