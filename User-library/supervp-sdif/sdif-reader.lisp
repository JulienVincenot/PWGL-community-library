...
(defmethod sdif-read-frame ((self 1FTD) stream)
  (let ((NbMatrix (read stream))
    ...
    (setf (NbMatrix self) NbMatrix
          ...
          (FrameData self)
          (sdif-read-matrixes FrameTime NbMatrix stream))
    self))
...
(defun sdif-read-matrixes(FrameTime NbMatrix stream)
  (loop repeat NbMatrix
        collect
        (let ((tag (read stream () :EOF)))
          (when (sdif-tag-p tag)
            (sdif-read-matrix2 FrameTime (make-instance (built-in-sdif-matrix-class tag) :Signature tag) stream)))))
...
; modification by hans

(defmethod sdif-read-matrix2(FrameTime (self 1MTD) stream)
  (let ((DataType (SDIF-type (read stream)))
        (NbRow (read stream))
        (NbCol (read stream)))
    (setf (DataType self) DataType
...
(defmethod sdif-read-matrix2(FrameTime (self 1MTD) stream)
  (let ((DataType (SDIF-type (read stream)))
    ...
    (setf (DataType self) DataType
          ...
          (MatrixData self)
                 (loop repeat NbRow collect
                     (ompw-utils::x-append (loop repeat NbCol collect (read stream)) FrameTime )))
    self))
