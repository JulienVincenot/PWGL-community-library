...
;(defun simple->tree (rhythmseq pitchseq timesignseq)
;  (let ((abs-rhythms (dx-x-pause-ok 1 (fuse-pauses-in-input (append rhythmseq '(-100))))))
;    (list (list (fuse-gracenotes (list (list (build-measure-seq abs-rhythms pitchseq timesignseq 0))))))))

(defun simple->tree-one-layer (rhythmseq pitchseq timesignseq nr-colored-measures tempo)
  (let ((abs-rhythms (dx-x-pause-ok 1 (fuse-pauses-in-input (append rhythmseq '(-100))))))
    (fuse-gracenotes (list (list (build-measure-seq abs-rhythms pitchseq timesignseq nr-colored-measures tempo))))))
...
(defun beats-lengths (measure)
  (remove nil (mapcar #'(lambda (x) (if (listp x)
                                        (car x)
                                      nil)) 
                      (beats-sublists measure))))
...
(defun re-combine-tree (measureinfos beatlengths sublists)
  (loop for measureinfo in measureinfos
        ...
        collect (append measureinfo (mapcar 'list measure-beat-lengths measure-beats)
                        (if (> (length measure-beats) (length measure-beat-lengths))
                            (subseq measure-beats (length measure-beat-lengths) (length measure-beats))))))
