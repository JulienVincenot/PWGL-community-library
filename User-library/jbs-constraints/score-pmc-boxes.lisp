...
(define-box s-pmc-index-allowed-harmony-rule ((harmony? (60 64 67))
  ...
  (when (eql parts :all) (setq parts nil))
  (case mode?    
    (:true/false `(:true/false * ,index :harmony ,@(when parts `(:parts ,parts))
    ...
    (:heuristic `(:heuristic * ,index :harmony ,@(when parts `(:parts ,parts))
                  (?if (let ((harm (sort (remove-duplicates (pw::g-mod (m ,index :complete? t) 12)) '<))
                             (harmony (sort (pw::g-mod ',harmony? 12) '<))) 
...
(define-box s-pmc-allowed-harmony-in-given-measures-rule ((harmony? (60 64 67))
  ...
  (when (eql parts :all) (setq parts nil))
  (case mode?    
    (:true/false `(:true/false * ?1 :measures ,measures ,@(when parts `(:parts ,parts))
                   (?if (member (pw::g-mod (m ?1) 12)
                                (pw::g-mod ',harmony? 12)))))
                        
    (:heuristic `(:heuristic * ?1 :measures ,measures ,@(when parts `(:parts ,parts))
...
(define-box s-pmc-allowed-harmony-on-beat-rule ((harmony? (60 64 67))
  ...
  (when (eql parts :all) (setq parts nil))
  (case mode?    
    (:true/false `(:true/false * ?1 :beats ,beats? ,@(when parts `(:parts ,parts))
                   (?if (member (pw::g-mod (m ?1 :complete? T) 12)
                                (pw::g-mod ',harmony? 12)))))
                        
    (:heuristic `(:heuristic * ?1 :beats ,beats? ,@(when parts `(:parts ,parts))
...
;
;;;==============================================================================================
;                               ALLOWED INTERVAL INSIDE PARTS RULE
;;;==============================================================================================
;
(define-box s-pmc-allowed-interval-inside-parts-rule ((intervals (1 5 7))
                                                      (parts ":all")
                                                      (mode? ":true/false")
                                                      (weight 1))
  :menu (mode? (:true/false "true/false") (:heuristic "heuristic"))
  :class score-pmc-box
  "This rule outputs a solution having only the interval entered in allowed-interval in a specific part set in parts. 
In the menu mode? you can chose if the rule is true /faulse
or heuristic.
ATTENTION: in parts if :all it means that the rule is vfor all the parts. If you want  specific part to be constrained
you have to put: :parts1, or :parts2..."
  (when (eql parts :all) (setq parts nil))
  (case mode?
    ...
    (:heuristic `(:heuristic * ?1 :chord ,@(when parts `(:parts ,parts))
                  (?if (if (let* ((harmony (m ?1 :complete? t :PARTS ,parts))
                                  (ints (pw::g-abs (pw::flat (jbs-constraints::find-all-intervals harmony)))))
                             (if ints (every #'(lambda (x) (member x ',intervals)) ints)
                               t)) ,weight 0))))))
...
;
;;;==============================================================================================
;                             NOT  ALLOWED INTERVAL INSIDE PARTS RULE
;;;==============================================================================================
;
(define-box s-pmc-not-allowed-interval-inside-parts-rule ((intervals (1 5 7))
                                                          (parts ":all")
                                                          (mode? ":true/false")
                                                          (weight 1))
  :menu (mode? (:true/false "true/false") (:heuristic "heuristic"))
  :class score-pmc-box
  "This rule outputs a solution NOT having the interval entered in allowed-interval in a specific part set in parts. 
In the menu mode? you can chose if the rule is true /faulse
or heuristic.
ATTENTION: in parts if :all it means that the rule isvfor all the parts. If you want  specific part to be constrained
you have to put: :parts1, or :parts2..."
  (when (eql parts :all) (setq parts nil))
  (case mode?
    (:true/false `(:true/false * ?1 :chord ,@(when parts `(:parts ,parts))
...
;
;;;==============================================================================================
;                                      BIGGER INTERVAL INSIDE PARTS RULE
;;;==============================================================================================
;
(define-box s-pmc-bigger-interval-inside-parts-rule ((interval 1)
                                                     (parts ":all")
                                                     (mode? ":true/false")
                                                     (weight 1))
  :menu (mode? (:true/false "true/false") (:heuristic "heuristic"))
  :class score-pmc-box
  "This rule outputs a solution having the intervals inside a part defined in parts bigger than the interval set in interval. 
In the menu mode? you can chose if the rule is true /faulse
or heuristic.
ATTENTION: in parts if :all it means that the rule isvfor all the parts. If you want  specific part to be constrained
you have to put: :parts1, or :parts2..."
  ...
  (case mode?
    (:true/false `(:true/false * ?1 :chord ,@(when parts `(:parts ,parts))
                   (?if (let* ((harmony (m ?1 :complete? t))
                               (int-max (pw::g-abs (pw::flat (jbs-constraints::find-all-intervals harmony)))))
                          (if int-max (every #'(lambda (x) (> x ,interval)) int-max)
                            t)))))



    (:heuristic `(:heuristic * ?1 :chord ,@(when parts `(:parts ,parts))
                  (?if (if (let* ((harmony (m ?1 :complete? t))
                                  (int-max (pw::g-abs (pw::flat (jbs-constraints::find-all-intervals harmony)))))
                             (if int-max (every #'(lambda (x) (> x ,interval)) int-max)
                               t)) ,weight 0))))))
;
;;;==============================================================================================
;                                      SMALLER INTERVAL INSIDE PARTS RULE
;;;==============================================================================================
;
(define-box s-pmc-smaller-interval-inside-parts-rule ((interval 1)
                                                      (parts ":all")
                                                      (mode? ":true/false")
                                                      (weight 1))
  :menu (mode? (:true/false "true/false") (:heuristic "heuristic"))
  :class score-pmc-box
  "This rule outputs a solution having the intervals inside a part defined in parts smaller than the interval set in interval. 
In the menu mode? you can chose if the rule is true /faulse
or heuristic.
ATTENTION: in parts if :all it means that the rule isvfor all the parts. If you want  specific part to be constrained
you have to put: :parts1, or :parts2..."
...
;
;;;==============================================================================================
;                                    ALLOWED HARMONY INSIDE PART RULE
;;;==============================================================================================
; 
(define-box s-pmc-allowed-harmony-inside-part-rule ((harmony? (60 64 67))
                                                    (parts ":all")
                                                    (mode? ":true/false")
                                                    (weight 1))
  :menu (mode? (:true/false "true/false") (:heuristic "heuristic"))
  :class score-pmc-box
  "This rule forces a part to  have a specific harmony set in harmony? (in modulo 12)"
  (when (eql parts :all) (setq parts nil))
  (case mode?    
    (:true/false `(:true/false * ?1 ,@(when parts `(:parts ,parts))
                   (?if (member (pw::g-mod (m ?1 :complete? T) 12)
...
;
;;;==============================================================================================
;                                   NOT  ALLOWED HARMONY INSIDE PART RULE
;;;==============================================================================================
;
(define-box s-pmc-not-allowed-harmony-inside-part-rule ((harmony? (60 64 67))
                                                        (parts ":all")
                                                        (mode? ":true/false")
                                                        (weight 1))
  :menu (mode? (:true/false "true/false") (:heuristic "heuristic"))
  :class score-pmc-box
  "This rule forces a part NOT have a specific harmony set in harmony? (in modulo 12)"
  (when (eql parts :all) (setq parts nil))
  (case mode?    
    (:true/false `(:true/false * ?1 ,@(when parts `(:parts ,parts))
                   (?if (not (member (pw::g-mod (m ?1 :complete? T) 12)
