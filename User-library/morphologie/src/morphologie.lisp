...
(define-box structure-2 ((seq nil) (n-max 10) (alpha? :alpha) (result :extended) &optional
			 (length nil) (seuil 10))
  "INPUT
...
(define-box direct-analysis ((list (60 40 69 73 61 59)))
...
(defun make-graph-tree (window tree nodes)
  ...
  (let ((ntree (test-tree (copy-list tree)))
	...
	(if (member (if (stringp (car coord))
		    ...
		    :test
		    #'equalp)
	    (set-fore-color window ccl::*blue-color*)
	    (set-fore-color window ccl::*black-color*))
...
"
  (let* ((elements (reverse (remove-duplicates (reverse sequence) :test #'equalp))) ; TODO use from-end t
